...
- ![image](https://github.com/user-attachments/assets/c929efe1-2431-4f31-ae98-242713bf5d35)
...
ตรงกับหลักคณิตศาสตร์ เพราะ 2147483647 คือค่าสูงสุดที่ชนิดข้อมูล int (32-bit signed) สามารถเก็บได้ จึงไม่มีการเกิดปัญหา overflow และค่าที่พิมพ์ออกมายังคงตรงตามที่กำหนด
...
ไม่ตรงกับหลักคณิตศาสตร์ เพราะ ค่าที่คำนวณได้คือ -2147483649 ซึ่ง ต่ำกว่าขอบเขต ที่ int รองรับ เกิด Integer Underflow ค่าจะ "วนกลับ" ไปยังค่าบวกสูงสุด (2147483647) หรือค่าอื่นๆ ขึ้นกับการทำงานของ two's complement
...
![image](https://github.com/user-attachments/assets/682b4b6a-176a-44b0-85f8-3146aa707f22)

__คำถาม__ 

3.1 คุณสังเกตเห็นความแตกต่างของความแม่นยำระหว่าง float และ double อย่างไร? 
ทั้ง float และ double มี ขนาดเท่ากัน (32 บิต)เก็บค่าทศนิยมได้ประมาณ 6–7 หลักเท่านั้นถึงจะใช้ double ก็ ไม่แม่นยำไปกว่า float (15 หลัก ถ้าเป็น 64 บิต)
 
3.2 สถานการณ์ใดที่คุณควรเลือกใช้ double แทน float?
ตอบ ต้องการความแม่นยำสูงกว่า 6–7 ตำแหน่งทศนิยม เช่น คำนวณเชิงวิทยาศาสตร์ หรือทางวิศวกรรมที่ต้องการความละเอียดสูง
...
- โค้ดที่ต้องเพิ่มใน `setup()` โดยลบโค้ดเดิมออก ยกเว้นบรรทัด `Serial.begin(115200);`
![image](https://github.com/user-attachments/assets/501d5a2b-f85d-42e0-8118-618c54530c10)
...
ตัวอักษรแต่ละตัวจะถูกแทนด้วย "รหัสตัวเลข" ตามมาตรฐานที่เรียกว่า ASCII
เช่น: 'A' = 65 'Z' = 90 'a' = 97 'z' = 122 '0' = 48 ' ' (ช่องว่าง) = 32

4.2 ถ้าอยากทราบความสัมพันธ์ระหว่างตัวเลขกับอักขระทั้งหมด สามารถหาได้จากเอกสารใด หรือแหล่งอ้างอิงใด
ตาราง ASCII (ASCII Table) เว็บไซต์ทั่วไป เช่น: asciitable.com

4.3 จากข้อ 4.2 นักเขียนโปรแกรมสามารถกำหนดขึ้นเองได้ หรือมีเอกสารใดกำกับอยู่
ไม่ได้กำหนดเอง เพราะ: ASCII เป็น มาตรฐานสากล ที่ถูกกำกับโดยเอกสารของ ANSI (American National Standards Institute)
